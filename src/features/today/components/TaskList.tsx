      ...
      {view.workItems.length === 0 ? (
        ...
        <ul className="space-y-2">
          {view.workItems.map((item) => {
            const matchingBlocks = view.timeBlocks.filter((candidate) => candidate.workItemId === item.id);
            const timeBlockId = matchingBlocks.length === 1 ? matchingBlocks[0].id : undefined;
            return (
              <li
                key={item.id}
              ...
              >
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  <Link
                    href={startFocusHref(item.id, timeBlockId)}
